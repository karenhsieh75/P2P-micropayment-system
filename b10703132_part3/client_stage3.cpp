...
SSL_CTX* client_send_ctx;
SSL* client_send_ssl;

SSL_CTX* client_receive_ctx;
SSL* client_receive_ssl;
...
void* listener_thread(void* args) {
    ...
    while (1) {
        ...
        SSL_CTX* p2p_ctx = init_server_ctx();
        X509* p2p_cert = generate_certificate(pkey);
        load_certificate(p2p_ctx, p2p_cert, pkey);

        cout << "load..." << endl;
        server_ssl = SSL_new(p2p_ctx);  // 基於 ctx 產生一個新的 SSL
        cout << "create ssl..." << endl;
        SSL_set_fd(server_ssl, client_fd);  // 將通道跟 socket fd 連結
        cout << "setting fd.." << endl;
        ...
        else
        {
            ...
            show_certificate(server_ssl);
        }


        // 接收訊息
        char buffer[BUFFER_SIZE];
        int bytes_received = SSL_read(server_ssl, buffer, BUFFER_SIZE);
        ...
        string plaintext = decrypt(string(buffer, bytes_received), pkey);
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        // Case 2: REGISTER#<UserAccountName> or List
            ...
            int bytes_received = SSL_read(ssl, response, BUFFER_SIZE);
            if (bytes_received > 0) {
                response[bytes_received] = '\0';
                string plaintext = decrypt(string(response, bytes_received), pkey);
                cout << plaintext << "-------------------------------" << endl;
            }
            else {
        ...
        // Case 4: <MyUserAccountName>#<payAmount>#<PayeeUserAccountName>
            ...
            int bytes_sent = SSL_write(ssl, ciphertext.c_str(), ciphertext.size());
            int bytes_received = SSL_read(ssl, list_response, BUFFER_SIZE);

            list_response[bytes_received] = '\0';
            string plaintext = decrypt(string(list_response, bytes_received), pkey);
            ...
            char* line = strtok(plaintext.data(), "\n");
            while (line) {
            ...
            cout << "P2P message sent!";


            // 等待 server 回應
            bytes_received = SSL_read(ssl, response, BUFFER_SIZE);
            if (bytes_received > 0) {
                response[bytes_received] = '\0';
                string plaintext = decrypt(string(response, bytes_received), pkey);
                cout << plaintext << "-------------------------------" << endl;
            ...
            bytes_received = SSL_read(ssl, list_response, BUFFER_SIZE);

            list_response[bytes_received] = '\0';
            plaintext = decrypt(string(list_response, bytes_received), pkey);
            cout << plaintext << "-------------------------------" << endl;
        }
    }
